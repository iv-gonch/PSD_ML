...
- `csv_data_processing.ipynb` is the primary reproducible CSV pipeline. Its first stage
  creates a seeded sample of 10,000 events: exactly 1,000 for every `source × channel`
  pair. It writes an aligned provenance table and ten channel-specific interactive HTML
  waveform plots under `gamma_n_data/samples/`; PNG generation is disabled. The same
  ten plots are stored directly in notebook outputs using Plotly MIME, without iframes.
...
- Canonical channel labels are: CH0/CH2 `PMT-9102B + T-Stlbn`, CH3
  ...
  `docs/DECISIONS.md`.
- No background (`B`) run, independent repeat runs, validated neutron labels,
  reproducible data-quality audit, feature pipeline, or trained classifier is present.
...
3. Implement the reproducible run/event audit and classical PSD baseline described in
   `docs/signal_processing_plan.md`, including rejection, saturation, baseline, timing,
   and energy-stratified diagnostics per channel.
4. Only after leakage checks, build interpretable shape features and compare them with
